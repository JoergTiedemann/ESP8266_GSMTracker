...
export function WifiPage(props) {
    ...
    function fetchData() {
        fetch(`${props.API}/api/wifi/get`)
            .then((response) => {
                //console.log("Wifi Response Json: ",response.json());
                return response.json();
            })
            .then((data) => {
                setState(data);
                console.log("WifiGetData: Data",data);
            });
      }

      function fetchScanData() {
        fetch(`${props.API}/api/wifi/getScan`)
            .then((response) => {
                //console.log("Wifi Response Json: ",response.json());
                return response.json();
            })
            .then((data) => {
                setState(data);
                console.log("WifiGetScanData: Data",data);
            });
      }
    ...
    if (state.captivePortal === true) {
        connectedTo = <>{loc.wifiCP} (<a onClick={(e) => {setShowWifi(true);fetchData();}}>{loc.globalShowWifi}</a>)</>;
    } else if (state.captivePortal === false) {
        connectedTo = <>{loc.wifiConn} {state.ssid} ({state.strength}dB) (<a onClick={() => setForgetModal(true)}>{loc.wifiForget}</a>) (<a onClick={(e) => {setShowWifi(true);fetchScanData();}}>{loc.globalShowWifi}-Scan</a>)</>;
    }
    ...
    if (ShowWifi) {
        // setShowWifi(false);
        // console.log("WifiScandaten:",state);
        if (state.wifiInfo.length  == 0) {
            page = <>{page}<p>keine Wlannetzwerke erkannt</p></>;
         } else {
             for (let i = 0; i < state.wifiInfo.length; i++) {
                page = <>{page}<p>{state.wifiInfo[i]}</p></>;
                 }   
            }    
    }
