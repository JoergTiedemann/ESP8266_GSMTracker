...
const int intPin = 15; // D8 
volatile bool motion = false;

void ICACHE_RAM_ATTR motionISR() {
    // Serial.println("Motion erkannt");
    motion = true;
  }
   

void CGyroManager::begin()
{
...
void CGyroManager::InitWakeOnMotion()
{
   ...
   */
  myMPU9250.setWakeOnMotionThreshold(32);  // 128 = ~0.5 g
...
bool CGyroManager::isMotionDetected()
{
  // if(motion)
  //   Serial.println("Motion erkannt!");
  return motion;
...
void CGyroManager::ResetMotion()
{
  if(motion){
    ...
    Serial.println("now Resetmotion and Motion Detection Ready !");
    }
}

void CGyroManager::loop()
{
    xyzFloat gValue;
    if((millis()%1000) == 0){
    ...
    // if(motion){
    //   Serial.println("Motion erkannt aus loop!");
    // }
